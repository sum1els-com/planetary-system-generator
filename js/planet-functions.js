import planetaryHydrosphere from './planetary-hydrosphere.js';
import planetaryTemperature from './planetary-temperature.js';
import resultHelper from './result-helper.js';
import rollHelper from './roll-helper.js';
import stellarCalculator from './stellar-calculation-helper.js';
...
function getDensity(code, zone) {
    let roll = rollHelper.roll({ rollFunction: localChance.d6, iterations: 2 });
    let density = "";
...
function getInclination(roll) {
    if(roll.total >= 18) {
        let inclination = rollHelper.roll(settings.inclinationValueRoll)
...
function getAtmosphere(planet) {
    ...
    // Roll
    let roll = rollHelper.roll({ rollFunction: chance.d6, iterations: 1, modifier: modifier, minRoll: 1, maxRoll: 6 });
...
function getHydrosphere(planet) {
    ...
    if(planet.code == "planet.class.g" || planet.code == "planet.class.l") {
        let dice = resultHelper.clone(planetaryHydrosphere.settings.dice);
        dice.rollFunction = localChance.d6;
        dice.modifier = -5;

        let hydrosphereRoll = planetaryHydrosphere.roll(dice);
        ...
        let ammonia = planet.atmosphericComposition.components.find(element => element.name == "Ammonia");
        let methane = planet.atmosphericComposition.components.find(element => element.name == "Methane");
...
function getTemperature(planet, zone) {
    ...
    if(planet.code == "planet.class.f" || planet.code == "planet.class.g"
            ...
            || planet.code == "planet.class.r") {
        let temperatureRoll = planetaryTemperature.roll();
        ...
        let dice = resultHelper.clone(planetaryTemperature.settings.dice);
        dice.rollFunction = localChance.d6;
        dice.minRoll = 2;
        dice.maxRoll = 12;

        let temperatureRoll = planetaryTemperature.roll(dice);
        temperature = temperatureRoll.planetaryTemperature;
    }
...
function getAtmosphericComposition(planet) {
    ...
    if(planet.atmosphere.code == "atmosphere.none" || planet.code == "planet.class.d") { return composition; }
        ...
        let majorComponents = rollHelper.roll({ rollFunction: localChance.d6, iterations: 1, modifier: -2, minRoll: 1 });

        for(let i=0; i<majorComponents.total; i++) {
            let component = getAtmosphericCompositionFn(planet, composition, planet.zone);
    ...
    while(composition.total < 100) {
        let component = { name: getTraceElement(), value: localChance.d6() };
        ...
        if(component.name == "Chlorine" && planet.code == "planet.class.h") { continue; }
            ...
            let existingComponent = composition.components.find(element => element.name == component.name)
...
function getAtmosphericCompositionFn(planet, composition, zone) {
    ...
    // Roll
    let roll = rollHelper.roll({ rollFunction: localChance.d6, iterations: 2, modifier: modifier });
...
function getLife(planet) {
    let life = { name: "None", code: "life.none", order: 0 };
    let rollDef = {
    ...
    if(planet.atmosphere != "atmosphere.none") {
        let oxygen = planet.atmosphericComposition.components.find(element => element.name == "Oxygen");
        let ammonia = planet.atmosphericComposition.components.find(element => element.name == "Ammonia");
        let chlorine = planet.atmosphericComposition.components.find(element => element.name == "Chlorine");
        let fluorine = planet.atmosphericComposition.components.find(element => element.name == "Fluorine");
        let sulfurDioxide = planet.atmosphericComposition.components.find(element => element.name == "Sulfur Dioxide");
        let methane = planet.atmosphericComposition.components.find(element => element.name == "Methane");
        // let carbonDioxide = planet.atmosphericComposition.components.find(element => element.name == "Carbon Dioxide");
...
export default planetFunctions;
