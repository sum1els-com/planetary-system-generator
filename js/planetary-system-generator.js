import orbitalDistanceGenerator from './orbital-distance-generator.js';
import planetGenerator from './planet-generator.js';
import planetaryAtmosphereHigh from './planetary-atmosphere-high.js';
import planetaryAtmosphereLow from './planetary-atmosphere-low.js';
import planetaryAtmosphereStandard from './planetary-atmosphere-standard.js';
import planetaryHydrosphere from './planetary-hydrosphere.js';
import planetarySystemStats from './planetary-system-stats.js';
...
import spectralClass from './spectral-class.js';
...
import stellarGiantType from './stellar-giant-type.js';
import stellarOrbitalSeparation from './stellar-orbital-separation.js';
import systemName from './system-name-generator.js';
...
function loadStarTypeInfo(classCode) {
    return fetch("./data/star-classes/star-classes-" + classCode + ".json")
        .then(response => response.json())
        .then(data => {
            starTypeInfo[classCode] = data;
            return data;
        })
        .catch(err => console.error("Error loading star type info: " + classCode, err));
}

function loadStarInfo() {
    return fetch("./data/stars.json")
        .then(response => response.json())
        .then(data => {
            starInfo = data;
            return data;
        })
        .catch(err => console.error("Error loading star info", err));
}
...
function generateSystemName() {
    let tempResult = systemName.roll();
...
function generateSystemType() {
    let tempResult = starSystem.roll();
    ...
    // Build Star List
    for(let i = 0; i < planetarySystem.type.value; i++) {
        let tempStar = { id: i };
...
function generateStarType(id) {
    let tempResult = stellarClassification.roll();
...
function groupStars() {
    let groupNumber = 0;
    let minOrbitalSeparation = 1000000;
    ...
    for(let i = 1; i < planetarySystem.stars.length; i++) {
        let star = planetarySystem.stars[i];
...
function generatePlanetarySystems() {
        ...
        if(group.stars.length > 1) {
            group.stars.forEach(function(star, sIndex) {
                ...
                star.orbits.series.forEach(function(orbit) {
                    if (orbit.zone != "N/A" && star.class.code != "table.spectralClass.d" && star.minPlanetaryDistance <= orbit.distance && orbit.distance <= star.maxPlanetaryDistance) {
            ...
            group.orbits.series.forEach(function(orbit) {
                if (orbit.zone != "N/A" && group.minPlanetaryDistance <= orbit.distance && orbit.distance <= group.maxPlanetaryDistance) {
            ...
            // Single Star Groups
            group.orbits.series.forEach(function(orbit) {
                if (orbit.zone != "N/A" && star.class.code != "table.spectralClass.d" && group.minPlanetaryDistance <= orbit.distance && orbit.distance <= group.maxPlanetaryDistance) {
    ...
    // Oort cloud
    let oortCloudRoll = rollHelper.roll(settings.oortCloudRoll);
...
const planetarySystemGenerator = {
