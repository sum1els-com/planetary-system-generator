...
function calculateOrbitalPeriod(solarMass, auRadius) {
    let r = auRadius * constants.auRadius;
    let gm = solarMass * constants.smGav;

    let r3 = Math.pow(r, 3);
    let op = Math.sqrt(4 * (Math.pow((Math.PI), 2) * r3 ) / gm);
    let nw = op / constants.smDivisor;
...
function calculateLunarOrbitalPeriod(planetMass, orbitalRadius) {
    let r = orbitalRadius; // km
    let gm = planetMass * constants.emGrav;

    let r3 = Math.pow(r, 3);
    let op = Math.sqrt(4 * (Math.pow((Math.PI), 2) * r3 ) / gm);
    let nw = op * constants.emDivisor;
...
export default stellarCalculator;
