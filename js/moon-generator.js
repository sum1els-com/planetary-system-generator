import planetFunctions from './planet-functions.js';
...
let settings = planetFunctions.settings;

function rollMoon(planet, zone) {
    let satellites = [];
    let rolls = {};
    let rollD, rollF, rollG, rollK, rollL;
    let moonCountClassD, moonCountClassF, moonCountClassG, moonCountClassK, moonCountClassL;
...
function rollMoonDetails(planet, code, zone) {
    ...
    let distance = getMoonDistance(code);
    let heat = null;
    ...
    let density = planetFunctions.getDensity(code, zone);
...
function getMoonDistance(code) {
    let firstRoll = rollHelper.roll({ rollFunction: localChance.d6, iterations: 1 });
