...
function getStatsPlanetFn(stats, planet) {
    let planetStat = stats.planets.find(element => element.code == planet.code);
...
function getStatsMoonFn(stats, moon) {
    let moonStat = stats.moons.find(element => element.code == moon.code);
...
const planetarySystemStats = {
    getStats: getStats
};

export default planetarySystemStats;
